...
# Check for required ALERT_API
if [ -z "$ALERT_API" ]; then
  echo "❌ ALERT_API not defined in config.env. Exiting."
  exit 1
fi
...
# Configure alert endpoint to file
  ...
  - name: 'slack'
    slack_configs:
      - channel: '#servers'
        ...
        api_url: $ALERT_API
EOF
echo "✅ alertmanager.yml generated for specified API."
